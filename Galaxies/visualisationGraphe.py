...
def sauveGrapheAmas_(numGalaxie, dictAmas) :
    dirAmas = shelve.open(parametres.DirBD + '/listeAmasGalaxie' + str(numGalaxie))
    for i in dictAmas :
        creerAmasTextes(dirAmas[str(i)], 'graphe_galaxie_'+str(numGalaxie)+'_amas_'+str(i),'gexf')
    dirAmas.close()
...
def creerAmasTextes(ListeNoeuds, fichier, ext) :
    ...
    for Noeud in ListeNoeuds:
        ...
        if Noeud != [] :
            ...
            # print(T)
            Graphe.add_node(Noeud, texte=L[0], longueurTexte=len(L[0]), auteur=T[0], titre=T[1], date=T[2], reutilisation=str(baseDonnees.reutilisations(int(Noeud))))
            curseur.execute('''SELECT idNoeudFils FROM grapheGalaxies WHERE idNoeudPere = (?)''', (Noeud,))
            S = curseur.fetchall()
            # print("Noeud: "+str(Noeud)+" - arcs incidents: "+str(L))
            if S!=[]:
    ...
    # print(Graphe.edges())
    if (ext=='gexf') :
        nx.write_gexf(Graphe, parametres.DirAmas+'/' +fichier+'.'+ext,encoding='utf-8',prettyprint=True, version='1.2draft')
    elif (ext=='gml') :
        nx.write_gml(Graphe, parametres.DirAmas+'/'+fichier+'.'+ext)
    # sauvJson(Graphe, parametres.DirAmas + '/' + fichier)
    connexion.close()
            ...
            for Fils in arcs[Pere]:
                if not Fils in Graphe:
                    ...
                    if RequetePere == True or filtres.filtreAffichage(requete, len(Ref[0]), T[0], T[1], T[2]):
                        LFils.append([str(Fils), Ref[0], len(Ref[0]), T[0], T[1], T[2], str(baseDonnees.reutilisations(Fils))])
            if LFils:
                Graphe.add_node(LPere[0], texte=LPere[1], longueurTexte=LPere[2], auteur=LPere[3], titre=LPere[4], date=LPere[5], reutilisation=LPere[6])
                for Noeud in LFils:
                    Graphe.add_node(Noeud[0], texte=Noeud[1], longueurTexte=Noeud[2], auteur=Noeud[3], titre=Noeud[4], date=Noeud[5], reutilisation=Noeud[6])
